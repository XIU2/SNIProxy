...
> - 虽然 HTTP/2 有个特性可以在单个 TCP 连接中复用多个域名，这看似会导致 SNIProxy 无法根据域名来正确转发流量，但好消息是现代浏览器只有在多个域名同时满足这些条件时才会复用连接：**`解析到相同的 IP(服务器) + 使用相同的 TLS 证书（如通配符证书或包含多个域名的证书）+ 相同的端口（如 443）`**，而对于这种情况是可以正常处理的（或者说压根不需要特殊处理，程序只负责转发流量，浏览器和服务器会协商处理）。
